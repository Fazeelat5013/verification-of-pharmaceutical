...
limiter = Limiter(
    get_remote_address,
    app=app,
    storage_uri=os.getenv("RATELIMIT_STORAGE_URL", None),
    default_limits=[os.getenv("DEFAULT_HOURLY_LIMIT", "200 per hour"), os.getenv("DEFAULT_MINUTE_LIMIT", "50 per minute")]
)
...
def admin_edit_medicine(mid):
    ...
    if request.method == 'POST':
        ...
        med.batch_no = request.form['batch_no']
        med.approved_date = request.form['approved_date']
        med.expiry_date = request.form['expiry_date']
        med.company_id = request.form['company_id']
        qr_file = request.files.get('qr_image')
        if qr_file:
            qr_filename = save_upload(qr_file)
    ...
    return render_template('admin_edit_medicine.html', medicine=med, companies=companies)

@app.post('/admin/medicines/<int:mid>/recall/remove')
@login_required
